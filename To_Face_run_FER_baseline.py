...
import argparse
import re
from ToFace_1 import ToFace, ToFace_Unet,ToFace_Unet_cnn, ClassificationNetwork, ToFace_Unet_3Dcnn,ClassificationNetwork_mobilenet,ToFace_Unet_3Dcnn_complex, Unet_baseline, Mobilenet_baseline
import pickle
...
def test(model,class_model, test_dataset, out_size, modelname, use_range_mask):
    ...
    with torch.no_grad():
        for i, (orientation,depth, tof_data, class_label,face_orientation, input_depth,input_reflectance,target,imgs,mask,uid) in enumerate(test_dataset):
            ...
            # classifier,face_orientation_output,uid_output,need_teach = class_model(class_input)
    ...
    return val_loss/l,1, total_pred, total_true, L1/l, L2/l, L3/l, L4/l, L5/l, L22/l
...
def train(data_path, modelname, out_size, use_range_mask):
    ...
    # test_dataset = physical_dataset_class_FER(data_path, users, False,['P1','P2'],[],True)
    ...
    for epoch in range(num_epoch):  
        ...
        for i, (orientation,depth, tof_data, class_label,face_orientation, input_depth,input_reflectance,target,imgs, mask, uid) in enumerate(train_dataset):
            ...
            # classifier,face_orientation_out,uid_out,need_teach = class_model(class_input)
            ...
            loss = l1 + l2 #+l42+l22#+0.1*l2_smooth#
            # loss = l4+l5
            # end_time = time.time()
...
if __name__ == '__main__':
    #python To_Face_run_FER_baseline.py --data_folder inves_dataset/dataset_FER/ --model ToFace_Unet
